...
##            if logger: logger.info("kngMovesL:{0}\n".format(self.kngMovesL))
    ...
    def fndMove(self, state, logger=None):
        ...
        """
        valu =  lambda x: 1 if x <=  0 else x +  1
        mov = [(tbl_topStt, tbl_topStt.crd.suit) for tbl_nme, tbl_topStt in state.tbl_topD.items()\
               if tbl_topStt.crd !=  Crd_EMPTY\
               and tbl_topStt.crd.valu == valu(state.fnd_topD[tbl_topStt.crd.suit].crd.valu)]
        self.fndMovesL = mov 
    ...
    def sibMove(self, state, logger=None):
        ...
        #topNotEmptyorAceRULE: len(crdOD[crd].loc.nme)  > 1
        #topNotEmptyorAceRULE: len(stkOD[nme])  > 1
        #sibFaceUpRULE : crdOD[sibCrd].fce (== True)
        ...
        sibFaceUpRULE =  lambda stt: stt.fce
        sibDiffTblRULE =  lambda topStt,  sibStt : topStt.loc.stk.nme !=  sibStt.loc.stk.nme
        
        not_mty_topCrd = ((state.stkOD[nme],  state.stkOD[nme].top_item) for nme in  TABLEAUS if topNotEmptyorAceRULE(nme))
        
        for topStk,  topCrd in  not_mty_topCrd :
            topStt = state.crdOD[topCrd]
            sibCrd = topCrd._replace(valu=topCrd.valu-1)
            sibStt = state.crdOD[sibCrd]
            if sibFaceUpRULE(sibStt) and sibDiffTblRULE(sibStt, topStt):
                mov =  Mov(sibCrd, topStk)
